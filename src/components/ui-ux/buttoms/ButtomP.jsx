...
function ButtomP({ url, title, colorletter, color, number, isActive }) {
	return (
		<div>
			<a
				href={url}
				className="flex items-center cursor-pointer transition-all ease-in-out duration-100 text-white/40 gap-5 group w-[300px] font-ubuntu"
			>
				<div className="flex items-center gap-5">
					{/* div rallita */}
					<div
						className={`transition-all h-[1px] duration-100 ease-in-out ${
							isActive ? 'w-[70px] bg-white' : 'w-[30px] group-hover:w-[50px] bg-slate-400'
						}`}
					></div>
					{/* div rallita */}
					<p
